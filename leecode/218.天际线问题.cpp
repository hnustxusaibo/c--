...
public:
    ...
    vector<vector<int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        que.push(pii(0, INT_MAX));
        int buildings_t=0;
        vector<vector<int>> ans = {};
        for(int i=0; i<x_vector.size(); i++){
            int val = x_vector[i];
            // printf("val=%d\n",val);
            while(buildings_t < buildings.size() && buildings[buildings_t][0] <= val) {
                que.push(pii(buildings[buildings_t][2],buildings[buildings_t][1])); 
                buildings_t++;
            }
            while(!que.empty() && que.top().second <= val) {
                // printf("pop=%d\n",que.top().second);
                que.pop();
            }
            int max_height = que.top().first;
            if (ans.empty() || ans.back()[1] != max_height) { // 避免插入重复的高度
                ans.push_back({val, max_height});
                printf("%d %d\n",val, que.top().first);
            }
        }
        return ans;
